...
import { z } from 'zod';
import { api } from '../../api';
import type { DirectorOrBeneficialOwner } from '../../api/types/onboarding';
...
function buildPrincipals(values: FormValues): DirectorOrBeneficialOwner[] {
  const director: DirectorOrBeneficialOwner = {
  ...
  const ubo: DirectorOrBeneficialOwner = {
    id: `principal_${Math.random().toString(36).slice(2, 10)}`,
...
export function DirectorsUboPage() {
  ...
  const [addedPrincipals, setAddedPrincipals] = useState<DirectorOrBeneficialOwner[]>(
  ...
  const mutation = useMutation({
    mutationFn: (principals: DirectorOrBeneficialOwner[]) => {
  ...
  function handleAddAnother(values: FormValues) {
    setAddedPrincipals((prev) => [...prev, ...buildPrincipals(values)]);
    reset();
  }

  function handleContinue(values: FormValues) {
    ...
    <OnboardingLayout stepIndex={1} title={directorsUboCopy.title} subtitle={directorsUboCopy.subtitle}>
      ...
      <form className="space-y-5" onSubmit={handleSubmit(handleContinue)} noValidate>
        ...
        <div className="flex justify-between gap-3 pt-2">
          <Button type="button" variant="outline" onClick={() => navigate('/onboarding/business-details')}>
